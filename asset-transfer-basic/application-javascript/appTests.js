...
async function test7(contract,  testNr){
    ...
    try{
        result = await contract.submitTransaction( 'reserveSeats', 'GladlyAbroad', 'BS0', JSON.stringify(["Viki Koste"]), 'viki@koste.com', 1);
        if (`${result}` !== '') {
            console.log(`*** Result: ${prettyJSONString(result.toString())}`);
        }
...
async function test11(contract, testNr){
    ...
    try{        
        ...
        result = await contract.evaluateTransaction('checkIn', 'Customer', reservationNr, [{cusName: 'Viki Koste', passport: 'OP123456'}]);
        console.log(`bookSeats: completed`);
    }catch{
        console.log(`bookSeats: failed`);
        testResult = 'FAILED';
